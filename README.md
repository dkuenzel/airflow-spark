...
* airflow-webserver: Airflow webserver and Scheduler.
    * Image: docker-airflow-spark:1.10.7_3.1.1
    * Port: 8282
...
* jupyter-spark: Jupyter notebook with pyspark for interactive development.
  * Image: jupyter/pyspark-notebook:3.1.1
  * Port: 8888
  * References: 
...
Inside the airflow-spark/docker/docker-airflow
    ...
    $ docker build --rm --force-rm -t docker-airflow-spark:1.10.7_3.1.1 .
...
Inside the airflow-spark/docker/docker-jupyter
    ...
    $ docker build --rm --force-rm -t jupyter/pyspark-notebook:3.1.1 .
...
Note: when running the docker-compose for the first time, the images postgres:9.6 and bitnami/spark:3.1.1 will be downloaded before the containers started.
...
You can increase the number of Spark workers just adding new services based on `bitnami/spark:3.1.1` image to the `docker-compose.yml` file like following:
...
Rebuild Dockerfile (in this example, adding GCP extra):
    ...
    $ docker build --rm --build-arg AIRFLOW_DEPS="gcp" -t docker-airflow-spark:1.10.7_3.1.1 .
